...
const containers = [
  container("a1", "web-frontend", "running"),
  container("b2", "api-server", "running"),
  container("c3", "redis-cache", "paused"),
  container("d4", "old-worker", "exited", new Date("2026-07-22T18:30:00Z")),
];

const meta = {
  title: "Components/ContainerDropdown",
  component: ContainerDropdown,
  render: (args) => ({
    components: { ContainerDropdown },
    setup: () => ({ args }),
    // dropdown-content only opens on focus; keep it forced open so the story
    // shows the container list.
    template: `<ContainerDropdown v-bind="args" class="dropdown-open">Related</ContainerDropdown>`,
  }),
} satisfies Meta<typeof ContainerDropdown>;
