<template>
  <!-- Shared settings body used by both the /settings route (stacked, default)
       and the fullscreen popup (columns). The route layout must stay identical,
       so the two-column arrangement is opt-in via the `columns` prop. -->
  <div class="@container">
    <div :class="columns ? 'grid items-start gap-8 @3xl:grid-cols-2' : 'flex flex-col gap-8'">
      <!-- ABOUT -->
      ...
      <!-- DISPLAY -->
      <section class="flex flex-col gap-4" :class="columns ? '@3xl:col-span-2' : ''">
        <div>
      ...
      <section class="flex flex-col gap-4">
        ...
        <div class="border-base-content/15 bg-base-200/40 divide-base-content/10 divide-y rounded-lg border">
          ...
          <label class="flex min-h-13 items-center justify-between gap-4 p-4 text-sm font-medium">
            <span class="inline-flex items-center gap-2">
              {{ $t("settings.open-as-popup") }}
              <span class="badge badge-warning badge-sm badge-outline">{{ $t("settings.experimental") }}</span>
            </span>
            <input type="checkbox" class="toggle toggle-primary toggle-sm" v-model="settingsAsPopup" />
          </label>
        </div>
      </section>
    </div>
...
const { columns = false } = defineProps<{ columns?: boolean }>();
